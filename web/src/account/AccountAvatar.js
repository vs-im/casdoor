...
class AccountAvatar extends React.Component {
  render() {
    const {src, size, width, height, borderRadius = "50%"} = this.props;
    ...
    if (size !== undefined) {
      return (
        <img width={size} height={size} src={src} style={{borderRadius}} />
      );
    } else if (width === undefined) {
      return (
        <img height={height} src={src} style={{borderRadius}} />
      );
    } else if (height === undefined) {
      return (
        <img width={width} src={src} style={{borderRadius}} />
      );
    }
  }
}
