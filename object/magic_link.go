...
type MagicLink struct {
	...
	// fork: the columns of the headless API and of the admin list, see magic_link_api.go
	MagicLinkExtension `xorm:"extends"`
}
...
func ConsumeMagicLink(token string, sessionHash string, application *Application, lang string) (*MagicLink, error) {
	...
	// fork: a link issued by the API carries its own expiry, see isMagicLinkExpired()
	if isMagicLinkExpired(magicLink, time.Now().Unix()) {
