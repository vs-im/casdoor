...
// @Title SendMagicLink
// @Tag Magic Link API
// @Description send a Magic Link to an email address for a validated application, organization, and permission context
...
func (c *ApiController) SendMagicLink() {
	...
	if requestForm.Email == "" || requestForm.Organization == "" || requestForm.Application == "" {
	...
	if application == nil {
		c.ResponseError(fmt.Sprintf(c.T("auth:The application: %s does not exist"), requestForm.Application))
		return
	}
	...
	if err != nil {
		c.saveFailedMagicLink(application, requestForm.Email, clientIP, oauth, err.Error())
	...
	if provider == nil {
		...
		c.saveFailedMagicLink(application, requestForm.Email, clientIP, oauth, errText)
	...
	if err != nil {
		link := object.NewMagicLink(application, nil, requestForm.Email, clientIP, c.GetSessionUsername(), "", oauth)
	...
	link := object.NewMagicLink(application, permission, requestForm.Email, clientIP, c.GetSessionUsername(), token, oauth)
	...
	err = object.SendMagicLinkToEmail(organization, provider, requestForm.Email, magicLinkURL)
	if err != nil {
	...
	util.LogInfo(c.Ctx, "Magic link email sent, organization = %s, application = %s, email = %s, magicLink = %s", application.Organization, application.Name, requestForm.Email, util.GetId(link.Owner, link.Name))
	c.ResponseOk()
}
...
func (c *ApiController) verifyMagicLinkCaptcha(application *object.Application, requestForm *MagicLinkRequestForm, required bool) (bool, error) {
	...
	if captchaProvider.Type != "Default" {
		requestForm.ClientSecret = captchaProvider.ClientSecret
	}
	isHuman, err := captcha.VerifyCaptchaByCaptchaType(requestForm.CaptchaType, requestForm.CaptchaToken, captchaProvider.ClientId, requestForm.ClientSecret, captchaProvider.ClientId2)
	if err != nil {
		return false, err
	}
	if !isHuman {
...
func (c *ApiController) saveFailedMagicLink(application *object.Application, email string, clientIP string, oauth map[string]string, lastError string) {
	link := object.NewMagicLink(application, nil, email, clientIP, c.GetSessionUsername(), "", oauth)
	link.Status = object.MagicLinkStatusFailed
...
// @Tag Magic Link API
// @Description verify a one-time Magic Link token and continue the login or OAuth flow
...
func (c *ApiController) validateMagicLinkOAuthContext(link *object.MagicLink) error {
	...
	for key, value := range expected {
		if c.Ctx.Input.Query(key) != value {
